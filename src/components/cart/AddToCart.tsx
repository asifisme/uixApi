          ...
          {cart.map((views) => (
            <li key={views.uid} className="flex py-2">
              <div className="  rounded-md border border-gray-200">
                <img
                  src={views.images[0]?.image || "https://placehold.co/600x400"}
                  alt={views.name}
                  className="h-16 w-full object-cover object-center"
                />
                <p className="text-center font-bold mt-2"> {views.quantity}</p>
              </div>
              <div className="ml-4 flex flex-1 flex-col">
                ...
                <div className="flex flex-1 items-end justify-between text-sm">
                  <div className="flex items-center gap-2 text-gray-500">
                    <button
                      onClick={() =>
                        updateQuantity(views.uid, views.quantity - 1)
                      }
                  ...
                  <div className="flex items-center gap-1">
                    <div onClick={() => removeFromCart(views.uid)} className="flex gap-2">
